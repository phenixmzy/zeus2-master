...
public class CardEditJob extends CenterTemplate implements
	...
	private TextButton save = new TextButton("保存", new SelectHandler() {
		...
		public void onSelect(final SelectEvent event) {
			...
			model.setSqlScript(sqlScript.getValue());//add by phenix
...
/*
	...
	public void refresh(final JobModel t) {
		...
		sqlScript.setValue(t.getSqlScript() == null ? "" : t.getSqlScript());//add by phenix
		if (JobModel.MapReduce.equals(t.getJobRunType())) {
			scriptFieldSet.hide();
			sqlScriptFieldSet.hide(); //add by phenix
			//hiveProcesserFieldSet.hide();
		} else {
			scriptFieldSet.show();
			//add by phenix
			if (JobModel.HS2.equals(t.getJobRunType())) {
				sqlScriptFieldSet.show();
			} else if (JobModel.RDBSQL.equals(t.getJobRunType())) {//add by phenix
				scriptFieldSet.hide();
				sqlScriptFieldSet.show();
				sqlScript.setValue(t.getSqlScript() == null ? "" : t.getSqlScript());
			} else {
	...
	public FieldSet getSqlScriptFieldSet() {
		if (sqlScriptFieldSet == null) {
			...
			sqlScriptFieldSet.setResize(false);
