...
public class JobServiceImpl implements JobService {
	...
	public JobModel createJob(String jobName, String parentGroupId,
		...
		} else if (JobModel.HS2.equals(jobType)) { //add by phenix
			type = JobRunType.Hs2;
		} else if (JobModel.GROOVY.equals(jobType)) { //add by phenix
			type = JobRunType.Groovy;
		} else if (JobModel.RDBSQL.equals(jobType)) { //add by phenix
			type = JobRunType.RdbSql;
		}
		log.error("JobServiceImpl.createJob = "+ type + " " + jobType + " " + jobType.length() + " " + JobModel.RDBSQL + " " + JobModel.RDBSQL.length());
		try {
	...
	public JobModel getUpstreamJob(String jobId) throws GwtException {
		...
		} else if (jobBean.getJobDescriptor().getJobType() == JobRunType.Hs2) { //add by phenix
			jobRunType = JobModel.HS2;
		} else if (jobBean.getJobDescriptor().getJobType() == JobRunType.Groovy) { //add by phenix
			jobRunType = JobModel.GROOVY;
		} else if (jobBean.getJobDescriptor().getJobType() == JobRunType.RdbSql) { //add by phenix
			jobRunType = JobModel.RDBSQL;
		} 
		...
		jobModel.setSqlScript(jobBean.getJobDescriptor().getSqlScript());//add by phenix
	...
	public JobModel updateJob(JobModel jobModel) throws GwtException {
		...
		if (jobModel.getJobRunType().equals(JobModel.MapReduce)) {
			...
			type = JobRunType.Hive;
		} else if (jobModel.getJobRunType().equals(JobModel.HS2)) { //add by phenix
			type = JobRunType.Hs2;
		} else if (jobModel.getJobRunType().equals(JobModel.GROOVY)) {//add by phenix
			type = JobRunType.Groovy;
		} else if (jobModel.getJobRunType().equals(JobModel.RDBSQL)) {//add by phenix
			type = JobRunType.RdbSql;
		}
		log.error("JobServiceImpl.updateJob=" + type);
		jd.setJobType(type);
		...
		jd.setSqlScript(jobModel.getSqlScript());
