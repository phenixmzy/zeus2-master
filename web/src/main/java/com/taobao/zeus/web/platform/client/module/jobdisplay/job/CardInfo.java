...
public class CardInfo extends CenterTemplate implements Refreshable<JobModel>{
	...
	private FieldSet sqlScriptFieldSet;//add by phenix
	...
	private CodeMirror sqlScript;//add by phenix
	...
	public CardInfo(JobPresenter presenter){
		...
		centerContainer.add(getSqlScriptFieldSet(),new MarginData(3));
	...
	private void display(final JobModel model){
		...
		if(JobModel.MapReduce.equals(model.getJobRunType())){
			...
			scriptFieldSet.show();
			if (JobModel.HS2.equals(model.getJobRunType())) {
				sqlScriptFieldSet.show();
				sqlScript.setValue(model.getSqlScript() == null ? "" : model.getSqlScript());//add by phenix
			} else if (JobModel.RDBSQL.equals(model.getJobRunType())) { //add by phenix
				scriptFieldSet.hide();
				sqlScriptFieldSet.show();
				sqlScript.setValue(model.getSqlScript() == null ? "" : model.getSqlScript());
			} else {
				sqlScriptFieldSet.hide();
			}
		}
	...
	//add by phenix
	public FieldSet getSqlScriptFieldSet() {
		if(sqlScriptFieldSet==null){
			sqlScriptFieldSet=new FieldSet();
			sqlScriptFieldSet.setCollapsible(true);
			sqlScriptFieldSet.setHeadingText("sql 脚本");
			CodeMirrorConfig cmc=new CodeMirrorConfig();
			sqlScript=new CodeMirror(cmc);
			sqlScriptFieldSet.add(sqlScript);
			sqlScriptFieldSet.setWidth("96%");
			sqlScriptFieldSet.addBeforeHideHandler(new BeforeHideEvent.BeforeHideHandler() {
				@Override
				public void onBeforeHide(BeforeHideEvent event) {
					if(sqlScriptFieldSet.getElement().getWidth(false)==0) {
						event.setCancelled(true);
					}
				}
			});
		}
		return sqlScriptFieldSet;
	}
}
