...
public class NewJobWindow extends Window{
	...
	public NewJobWindow(PlatformBus bus,GroupModel groupModel){
		...
		hs2.put("name", JobModel.HS2);
		jobTypeStore.add(hs2);
		
		//add by phenix
		Map<String, String> groovy=new HashMap<String, String>();
		groovy.put("name", JobModel.GROOVY);
		jobTypeStore.add(groovy);
		
		//add by phenix
		Map<String, String> rdbsql=new HashMap<String, String>();
		rdbsql.put("name", JobModel.RDBSQL);
		jobTypeStore.add(rdbsql);
