...
public class JobValidate {
	...
	public boolean valide(JobDescriptor job) throws ZeusException{
		...
		} else if (job.getJobType() == JobRunType.Hs2) {//add by phenix
			if(job.getScript()==null){
				throw new ZeusException("HS2 脚本不得为空");
			}
		} else if (job.getJobType() == JobRunType.Groovy) {//add by phenix
			if(job.getScript()==null){
				throw new ZeusException("Groovy 脚本不得为空");
			}
		} else if (job.getJobType() == JobRunType.RdbSql) {//add by phenix
			if(job.getSqlScript()==null){
				throw new ZeusException("SQL 脚本不得为空");
			}
		}
