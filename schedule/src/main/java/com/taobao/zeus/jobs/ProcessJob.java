...
public abstract class ProcessJob extends AbstractJob implements Job {
     ...
     public Integer run() throws Exception{
    	  logConsole("ProcessJob.run");
          int RETRIES_NUM = Integer.parseInt(jobContext.getProperties().getProperty("retries","3"));
          long retryBackoff  = Long.parseLong(jobContext.getProperties().getProperty("retry.backoff","1000"));
          int exitCode = -999;
          
          logConsole("Attempting Task... If task fail will retries " + RETRIES_NUM);
          exitCode = runExecutor();
          logConsole("Attempting Task... exitCode " + exitCode);
          
          if (exitCode != 0 && 
        		  (getJobContext().getRunType() == JobContext.SCHEDULE_RUN || 
        		  getJobContext().getRunType() == JobContext.MANUAL_RUN)) {
               for (int retries = 0; retries < RETRIES_NUM; retries++) {
            	   synchronized(this) {
            		   logConsole("Task delayed " + retryBackoff + " ms. Start wait at " + TIME_FORMAT.format(System.currentTimeMillis()));
            		   this.wait(retryBackoff);
            		   logConsole("Task delayed " + retryBackoff + " ms. Finsh wait at " + TIME_FORMAT.format(System.currentTimeMillis()));
            		   
            		   logConsole("Task failed. Retries " + retries);
                       exitCode = runExecutor();
                       logConsole("Attempting Failed Task... exitCode " + exitCode);
                        
                        if (exitCode == 0) {
                        	logConsole("Task success.");
                        	return exitCode;
                        }
     ...
     public Integer runExecutor() throws Exception{
